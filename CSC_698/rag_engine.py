...
def build_database():
    ...
    but get_retriever() will also call it automatically the first time
    if the database directory doesn't exist.
    """
...
def get_retriever():
    ...
    If the database directory does not exist, this will automatically
    build it once using the CSV files.
    """
    # If DB directory is missing, build it automatically
    if not os.path.exists(DB_DIR):
        print("ChromaDB not found on disk. Building it now...")
        build_database()
        if not os.path.exists(DB_DIR):
            # Still missing after build attempt → give a clear error
            raise RuntimeError(
                "Failed to build ChromaDB. Make sure the CSV files exist "
                "and run get_data.py first."
            )
